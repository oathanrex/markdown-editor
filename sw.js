...
self.addEventListener('install', (event) => {
    event.waitUntil(
        caches.open(CACHE_NAME).then((cache) => {
            ...
            // Cache CDN assets (with fallback)
...
// Fetch event - network first (fresh content on every load), cache fallback (offline)
self.addEventListener('fetch', (event) => {
    ...
    event.respondWith(
        fetch(event.request)
            .then((response) => {
                // Keep the cache updated with the fresh response
                if (response.ok) {
