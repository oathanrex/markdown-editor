...
export class App {
    ...
    async init() {
        ...
        try {
            ...
            // Sync --app-height with the visual viewport (mobile keyboard fix)
            this.setupViewportHeight();

            // Snap the app shell back if the browser focus-scrolls it
    ...
    setupEventListeners() {
        ...
        // Zen mode exit (floating button - the only exit when the toolbar is hidden)
        ...
        document.getElementById('sidebar-overlay')?.addEventListener('click', () => {
            ...
            if (sidebar?.classList.contains('open')) this.toggleSidebar();
        });

        // Close mobile sidebar with Escape key (WCAG 2.1.2)
    ...
    setupScrollSync() {
        ...
        const syncEditorToPreview = this.modules.performance.throttle(() => {
            ...
            const percentage = editorRange > 0 ? editor.scrollTop / editorRange : 0;
        ...
        const syncPreviewToEditor = this.modules.performance.throttle(() => {
            ...
            const editorRange = editor.scrollHeight - editor.clientHeight;
            const percentage = previewRange > 0 ? preview.scrollTop / previewRange : 0;
            if (editorRange > 0) {
                editor.scrollTop = percentage * editorRange;
            }
    ...
    setupResizeHandle() {
        ...
        const applyWidth = (newWidth) => {
            ...
            if (newWidth >= minWidth && newWidth <= maxWidth) {
                ...
                // Keep the focusable separator's ARIA value in sync (axe/a11y)
        ...
        handle.addEventListener('pointermove', (e) => {
            if (!isResizing) return;
            applyWidth(startEditorWidth + (e.clientX - startX));
        });
        ...
        // Keyboard resize support (WCAG 2.1.1)
        handle.addEventListener('keydown', (e) => {
            if (e.key !== 'ArrowLeft' && e.key !== 'ArrowRight') return;
            e.preventDefault();
            const step = e.shiftKey ? 48 : 16;
            const delta = e.key === 'ArrowRight' ? step : -step;
            applyWidth(editorPane.offsetWidth + delta);
        });
    }

    /**
     * Keep --app-height in sync with the visual viewport.
     * Fixes the "half screen missing" mobile glitch: 100dvh does not
     * shrink for the on-screen keyboard on iOS, and Android only
     * resizes the visual viewport, leaving bottom content unreachable.
     */
    setupViewportHeight() {
        const setAppHeight = () => {
            const raw = window.visualViewport
                ...
                : window.innerHeight;
            // Clamp so a transient 0-height report can never collapse the app
            const h = Math.max(raw, 150);
    ...
    /**
     * Safety net: browsers can programmatically scroll the app shell
     * (overflow: hidden does NOT prevent focus-scroll, e.g. when the
     * on-screen keyboard opens), which pushes the header out of view
     * with no scrollbar to come back. Snap the shell back instantly.
     */
    setupScrollGuard() {
        const reset = (e) => {
            const target = e.target;
            if ((target === document.body || target === document.documentElement) && target.scrollTop !== 0) {
                target.scrollTop = 0;
            }
        };
        // Capture phase: catches non-bubbling scroll events on body/html
    ...
    async handleTabChange(tab) {
        ...
        if (tab.content === null) {
            try {
                ...
                if (doc) {
                    tab.content = await this.getDisplayContentForDocument(doc);
                    tab.encrypted = !!doc.encrypted;
                    ...
                    this.showReadOnlyErrorDocument(
                        'Could not load document content from storage.\n\nReload the page or check console errors.'
                    );
        ...
        this.modules.history.startAutoSnapshot(
            tab.id,
            () => this.modules.editor.getContent(),
            { encrypt: !!tab.encrypted }
        );
    ...
    handleTOCNavigate(id) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (match) {
                // Replicate GitHub-style duplicate slugs (id, id-1, ...) so
                // TOC navigation works for repeated heading titles
                const base = match[1].trim().toLowerCase()
                    .replace(/[^\w\s-]/g, '')
                    .replace(/\s/g, '-') || 'section';
                const count = slugCounts.get(base) || 0;
                slugCounts.set(base, count + 1);
                const headingId = count === 0 ? base : `${base}-${count}`;
                if (headingId === id) {
                    lineNumber = i + 1;
                    break;
                }
    ...
    /**
     * Resolve decrypted/plaintext content for UI display
     */
    async getDisplayContentForDocument(doc) {
        if (!doc?.encrypted) {
            return doc?.content || '';
        }

        if (!this.modules.encryption.isSupported()) {
            throw new Error('Encryption is not supported in this browser');
        }

        try {
            return await this.modules.encryption.decrypt(doc.content);
        } catch {
            const passphrase = prompt('This document is encrypted. Enter passphrase to unlock it:');
            if (passphrase === null) {
                throw new Error('Decryption cancelled');
            }

            const content = await this.modules.encryption.decrypt(doc.content, passphrase);
            await this.modules.encryption.setPassphrase(passphrase);
            return content;
        }
    }
    ...
    /**
     * Show a non-persisted read-only error document in editor pane
     */
    showReadOnlyErrorDocument(message) {
        const display = `Error\n\n${message}`;
        this.modules.editor.setContent(display, { silent: true });
        this.modules.editor.setReadOnly(true);
        this.updateCounts(display);
    ...
    async saveCurrentDocument() {
        const activeTab = this.modules.tabs.getActiveTab();
        if (!activeTab) return;
        if (this.modules.editor.textarea?.readOnly) return;
        ...
        try {
            ...
            if (this.settings.encryptionEnabled) {
                if (!this.modules.encryption.isUnlocked) {
                    throw new Error('Encryption is enabled but locked. Set passphrase before saving.');
                }
            ...
            activeTab.encrypted = isEncrypted;
    ...
    toggleZenMode() {
        ...
        // Floating exit is the only clickable way out of zen mode
        if (exitBtn) {
            exitBtn.hidden = !isZen;
        }

        this.modules.accessibility?.announce(
            isZen ? 'Zen mode on. Press Escape or use the Exit Zen button to go back.' : 'Zen mode off.',
            'assertive'
        );
    ...
    toggleSidebar() {
        ...
        // Mobile overlay: visible only while the sidebar drawer is open
        if (overlay) {
            overlay.hidden = !(isOpen && sidebar?.classList.contains('open'));
        }
